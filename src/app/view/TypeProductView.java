...
public class TypeProductView extends javax.swing.JFrame {
    ...
    ArrayList<ProductType> listTypeProduct = new ArrayList<>();
    DefaultTableModel tableModel = new DefaultTableModel();
    ProductTypeService service = new ProductTypeService();

    public TypeProductView() {
        initComponents();
        listTypeProduct = service.getAll();
        tblDisplay.setModel(tableModel);
        addColumnTable();
        fillTable(listTypeProduct);
    }

    private void fillTable(ArrayList<ProductType> list) {
        int stt = 0;
        tableModel.setRowCount(0);
        for (ProductType prdType : list) {
            stt++;
            Object[] row = {stt, prdType.getCode(), prdType.getName()};
    ...
    private void addColumnTable() {
        ...
        tableModel.addColumn("Mã Dòng SP");
        tableModel.addColumn("Tên Dòng SP");
    }
    
    private ProductType inputProductType(){
        String typeProductName = "";
        if (txtNameCompany.getText().isEmpty()) {
            labelNameCompanyError.setText("Trường Này Không Được Để Trống");
            return null;
        }else{
            typeProductName = txtNameCompany.getText();
            labelNameCompanyError.setText("");
            ProductType prdType = new ProductType(typeProductName);
            return prdType;
        }
    }
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ProductType prdType = inputProductType();
        if (prdType != null) {
            service.add(prdType);
            refreshTable();
